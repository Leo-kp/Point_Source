...
def create_cube_mesh_p(
    ...
    gmsh.option.setNumber("Mesh.MeshSizeFromPoints", 0) #unlocks refiniment
    ...
    p_center = gmsh.model.occ.addPoint(width/2, height/2, center_z, mesh_size) 
...
#------------------------------------------------ local refining
    if local_ref:   
        ...
        gmsh.model.mesh.field.setNumbers(dist_field, "PointsList", [p_center])

        thresh_field = gmsh.model.mesh.field.add("Threshold")
        gmsh.model.mesh.field.setNumber(thresh_field, "InField", dist_field)
        gmsh.model.mesh.field.setNumber(thresh_field, "SizeMin", mesh_size/20)  
        gmsh.model.mesh.field.setNumber(thresh_field, "SizeMax", mesh_size) 

        d_min = (mesh_size / 20) * 2
        limit_dmax = min(width / 7, height/7,thickness/7)
        safe_dmax = mesh_size * 3.0 
        d_max = max(min(limit_dmax, safe_dmax), mesh_size * 2)
        gmsh.model.mesh.field.setNumber(thresh_field, "DistMin", d_min)
        gmsh.model.mesh.field.setNumber(thresh_field, "DistMax", d_max)  
...
def create_cube_mesh_psc(
    ...
    p_center = gmsh.model.occ.addPoint(width/2, height/2, center_z,0.0) 
    pts = [gmsh.model.occ.addPoint(x, y, z, 0.0) for x, y, z in coords]
    gmsh.model.occ.synchronize()
...
#------------------------------------------------ local refining
    if local_ref:   
        ...
        thresh_field = gmsh.model.mesh.field.add("Threshold")
        gmsh.model.mesh.field.setNumber(thresh_field, "InField", dist_field)
        gmsh.model.mesh.field.setNumber(thresh_field, "SizeMin", target_rw)  
        gmsh.model.mesh.field.setNumber(thresh_field, "SizeMax", mesh_size) 


        d_max = max(target_rw * 100, min(width / 20, height/20,thickness/20))
        
        gmsh.model.mesh.field.setNumber(thresh_field, "DistMin", target_rw * 5)
        gmsh.model.mesh.field.setNumber(thresh_field, "DistMax", d_max)
